...
public class    WaitingQueueService {
    ...
    public WaitingQueue joinQueueIfAbsent(Long eventId, Users user) {
        Long userId = user.getUserId();
        String queueKey = getQueueKey(eventId); // e.g. "queue:event:3"
        String statusKey = getStatusKey(eventId, userId); // e.g. "queue:status:3:42"
        ...
        List<String> fullQueue = redisTemplate.opsForList().range(queueKey, 0, MAX_PROCESSING_USERS+1);
        long processingCount = 0;
        if (fullQueue != null) {
            for (String id : fullQueue) {
                String key = getStatusKey(eventId, Long.parseLong(id));
                String status = redisTemplate.opsForValue().get(key);
                if ("PROCESSING".equals(status)) processingCount++;
            }
        }
